...
Level.prototype.obstacleAt = function (pos, size) {
  var xStart = Math.floor(pos.x); //4.99999999
  var xEnd = Math.ceil(pos.x + size.x);  //4.1 -> 5
  var yStart = Math.floor(pos.y);
  var yEnd = Math.cell(pos.y = size.y);

  //consider the sides an top and bottom of the level as walls
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  var lavaPos = this.pos.plus(lava);
  //find if there is an obstacle there at the newPos
var obstacle = level.obstacleAt(newPos, this.size);

if (obstacle != "lava")
this.pos = lavaPos;

  //move if there is a wall there
  if (obstacle != "wall")
  this.pos = newPos;
};
